...
exports.PumpManagerImpl = void 0;
const logger_js_1 = require("./logger.js");
const public_js_1 = require("./models/public.js");
const partitionPump_js_1 = require("./partitionPump.js");
/**
...
class PumpManagerImpl {
    ...
    async createPump(startPosition, connectionContext, partitionProcessor, abortSignal) {
        ...
        if (abortSignal.aborted) {
            logger_js_1.logger.verbose(`${this._eventProcessorName}] The subscription was closed before creating the pump for partition ${partitionId}.`);
            return;
        }
        ...
        if (existingPump) {
            if (existingPump.isReceiving) {
                logger_js_1.logger.verbose(`[${this._eventProcessorName}] [${partitionId}] The existing pump is running.`);
                return;
            }
            logger_js_1.logger.verbose(`[${this._eventProcessorName}] [${partitionId}] The existing pump is not running.`);
            await this.removePump(partitionId, public_js_1.CloseReason.OwnershipLost);
        }
        logger_js_1.logger.verbose(`[${this._eventProcessorName}] [${partitionId}] Creating a new pump.`);
        const pump = new partitionPump_js_1.PartitionPump(connectionContext, partitionProcessor, startPosition, this._options);
        try {
            ...
            logger_js_1.logger.verbose(`[${this._eventProcessorName}] [${partitionId}] An error occured while adding/updating a pump: ${err}`);
    ...
    async removePump(partitionId, reason) {
        try {
            ...
            if (pump) {
                ...
                logger_js_1.logger.verbose(`[${this._eventProcessorName}] [${partitionId}] Stopping the pump.`);
                await pump.stop(reason);
            }
            else {
                logger_js_1.logger.verbose(`[${this._eventProcessorName}] [${partitionId}] No pump was found to remove.`);
            }
        }
        catch (err) {
            logger_js_1.logger.verbose(`[${this._eventProcessorName}] [${partitionId}] An error occured while removing a pump: ${err}`);
            (0, logger_js_1.logErrorStackTrace)(err);
        }
    }
    /**
    ...
    async removeAllPumps(reason) {
        ...
        logger_js_1.logger.verbose(`[${this._eventProcessorName}] Removing all pumps due to reason ${reason}.`);
        const tasks = [];
        ...
        try {
            ...
            logger_js_1.logger.verbose(`[${this._eventProcessorName}] An error occured while removing all pumps: ${err}`);
            (0, logger_js_1.logErrorStackTrace)(err);
        }
        finally {
...
exports.PumpManagerImpl = PumpManagerImpl;
