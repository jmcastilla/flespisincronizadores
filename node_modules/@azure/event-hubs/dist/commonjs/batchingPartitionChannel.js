...
class BatchingPartitionChannel {
    _eventQueue = new awaitableQueue_js_1.AwaitableQueue();
    _batchedEvents = [];
    _bufferCount = 0;
    _readyQueue = [];
    _flushState = {
    ...
    async enqueueEvent(event) {
        ...
        if (!this._isRunning) {
            ...
            this._startPublishLoop().catch((e) => {
                logger_js_1.logger.error(`The following error occurred during batch creation or sending: ${JSON.stringify(e, undefined, "  ")}`);
            });
        }
    }
    /**
        ...
        while (!this._loopAbortSignal.aborted) {
            try {
                ...
                const event = eventToAddToBatch ??
                    (await (0, core_util_2.cancelablePromiseRace)([
