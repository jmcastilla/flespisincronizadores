...
const partitionProcessor_js_1 = require("./partitionProcessor.js");
const eventPosition_js_1 = require("./eventPosition.js");
const pumpManager_js_1 = require("./pumpManager.js");
const logger_js_1 = require("./logger.js");
const public_js_1 = require("./models/public.js");
const delayWithoutThrow_js_1 = require("./util/delayWithoutThrow.js");
...
class EventProcessor {
    ...
    constructor(_consumerGroup, _context, _subscriptionEventHandlers, _checkpointStore, options) {
        ...
        if (options.ownerId) {
            ...
            this._id = (0, utils_js_1.getRandomName)();
            logger_js_1.logger.verbose(`Starting event processor with autogenerated ID ${this._id}`);
        }
        this._eventHubName = this._context.config.entityPath;
        this._fullyQualifiedNamespace = this._context.config.host;
        this._processorOptions = options;
        this._pumpManager =
            options.pumpManager || new pumpManager_js_1.PumpManagerImpl(this._id, this._processorOptions);
    ...
    async _claimOwnership(ownershipRequest, abortSignal) {
        if (abortSignal.aborted) {
            logger_js_1.logger.verbose(`[${this._id}] Subscription was closed before claiming ownership of ${ownershipRequest.partitionId}.`);
            return;
        }
        logger_js_1.logger.info(`[${this._id}] Attempting to claim ownership of partition ${ownershipRequest.partitionId}.`);
        try {
            ...
            logger_js_1.logger.info(`[${this._id}] Successfully claimed ownership of partition ${ownershipRequest.partitionId}.`);
            await this._startPump(ownershipRequest.partitionId, abortSignal);
        }
        catch (err) {
            logger_js_1.logger.warning(`[${this._id}] Failed to claim ownership of partition ${ownershipRequest.partitionId}`);
    ...
    async _startPump(partitionId, abortSignal) {
        if (abortSignal.aborted) {
            logger_js_1.logger.verbose(`[${this._id}] The subscription was closed before starting to read from ${partitionId}.`);
            return;
        }
        if (this._pumpManager.isReceivingFromPartition(partitionId)) {
            logger_js_1.logger.verbose(`[${this._id}] There is already an active partitionPump for partition "${partitionId}", skipping pump creation.`);
            return;
        }
        logger_js_1.logger.verbose(`[${this._id}] [${partitionId}] Calling user-provided PartitionProcessorFactory.`);
        const partitionProcessor = new partitionProcessor_js_1.PartitionProcessor(this._subscriptionEventHandlers, this._checkpointStore, {
            fullyQualifiedNamespace: this._fullyQualifiedNamespace,
            eventHubName: this._eventHubName,
            consumerGroup: this._consumerGroup,
        ...
        logger_js_1.logger.verbose(`[${this._id}] PartitionPump created successfully.`);
    }
    async _getStartingPosition(partitionIdToClaim) {
        ...
        logger_js_1.logger.verbose(`No checkpoint found for partition ${partitionIdToClaim}. Looking for fallback.`);
    ...
    async _runLoopForSinglePartition(partitionId, abortSignal) {
        while (!abortSignal.aborted) {
            try {
                ...
                logger_js_1.logger.verbose(`[${this._id}] Pausing the EventProcessor loop for ${this._loopIntervalInMs} ms.`);
                // swallow errors from delay since it's fine for delay to exit early
                await (0, delayWithoutThrow_js_1.delayWithoutThrow)(this._loopIntervalInMs, abortSignal);
            }
        }
    ...
    async _runLoopWithLoadBalancing(loadBalancingStrategy, abortSignal) {
        ...
        while (!abortSignal.aborted) {
            ...
            try {
                ...
                logger_js_1.logger.verbose(`[${this._id}] Pausing the EventProcessor loop for ${delayDurationInMs} ms.`);
                // Swallow the error since it's fine to exit early from the delay.
                await (0, delayWithoutThrow_js_1.delayWithoutThrow)(delayDurationInMs, abortSignal);
            }
        }
    ...
    async _handleSubscriptionError(err) {
        ...
        if (this._subscriptionEventHandlers.processError) {
            try {
                ...
                logger_js_1.logger.verbose(`[${this._id}] An error was thrown from the user's processError handler: ${errorFromUser}`);
            }
        }
    }
    /**
    ...
    start() {
        if (this._isRunning) {
            logger_js_1.logger.verbose(`[${this._id}] Attempted to start an already running EventProcessor.`);
            return;
        }
        this._isRunning = true;
        this._abortController = new AbortController();
        logger_js_1.logger.verbose(`[${this._id}] Starting an EventProcessor.`);
        if (this._processingTarget) {
            logger_js_1.logger.verbose(`[${this._id}] Single partition target: ${this._processingTarget}`);
            this._loopTask = this._runLoopForSinglePartition(this._processingTarget, this._abortController.signal);
        }
        else {
            logger_js_1.logger.verbose(`[${this._id}] Multiple partitions, using load balancer`);
    ...
    async stop() {
        ...
        try {
            ...
            logger_js_1.logger.verbose(`[${this._id}] An error occurred while stopping the EventProcessor: ${err}`);
        }
        finally {
            logger_js_1.logger.verbose(`[${this._id}] EventProcessor stopped.`);
        }
        if (this._processingTarget) {
            logger_js_1.logger.verbose(`[${this._id}] No partitions owned, skipping abandoning.`);
        }
        else {
    ...
    async abandonPartitionOwnerships() {
        logger_js_1.logger.verbose(`[${this._id}] Abandoning owned partitions`);
...
function getStartPosition(partitionIdToClaim, startPositions) {
    if (startPositions == null) {
        return eventPosition_js_1.latestEventPosition;
    }
    if ((0, eventPosition_js_1.isEventPosition)(startPositions)) {
