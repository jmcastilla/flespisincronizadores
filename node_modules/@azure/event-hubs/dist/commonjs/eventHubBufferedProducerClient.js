...
class EventHubBufferedProducerClient {
     ...
     */
    _partitionAssigner = new partitionAssigner_js_1.PartitionAssigner();
    /**
    ...
    constructor(fullyQualifiedNamespaceOrConnectionString1, eventHubNameOrOptions2, credentialOrOptions3, options4) {
        if (typeof eventHubNameOrOptions2 !== "string") {
            this.identifier = eventHubNameOrOptions2.identifier ?? (0, utils_js_1.getRandomName)();
            this._producer = new eventHubProducerClient_js_1.EventHubProducerClient(fullyQualifiedNamespaceOrConnectionString1, {
            ...
            this.identifier = credentialOrOptions3?.identifier ?? (0, utils_js_1.getRandomName)();
            this._producer = new eventHubProducerClient_js_1.EventHubProducerClient(fullyQualifiedNamespaceOrConnectionString1, eventHubNameOrOptions2, { ...credentialOrOptions3, identifier: this.identifier });
            this._clientOptions = { ...credentialOrOptions3 };
        }
        else {
            this.identifier = options4?.identifier ?? (0, utils_js_1.getRandomName)();
            this._producer = new eventHubProducerClient_js_1.EventHubProducerClient(fullyQualifiedNamespaceOrConnectionString1, eventHubNameOrOptions2, credentialOrOptions3, { ...options4, identifier: this.identifier });
    ...
    async enqueueEvent(event, options = {}) {
        ...
        if (!this._isBackgroundManagementRunning) {
            ...
            this._startPartitionIdsUpdateLoop().catch((e) => {
                logger_js_1.logger.error(`The following error occurred during partition ID update loop: ${JSON.stringify(e, undefined, "  ")}`);
            });
    ...
    _getPartitionChannel(partitionId) {
        const partitionChannel = this._partitionChannels.get(partitionId) ??
            new batchingPartitionChannel_js_1.BatchingPartitionChannel({
