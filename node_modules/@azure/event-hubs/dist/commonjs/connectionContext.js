    ...
    function create(config, tokenCredential, options) {
        ...
        async function handleDisconnection(context) {
            ...
            try {
                ...
                if (context.connection && context.connection.error) {
                    logger_js_1.logger.verbose("[%s] Accompanying error on the context.connection: %O", connectionContext.connection.id, context.connection.error);
                }
                if (context.error) {
        ...
        const onDisconnected = (context) => {
            ...
            handleDisconnection(context).catch((err) => {
                logger_js_1.logger.verbose(`[${connectionContext.connectionId}] Unhandled error in 'disconnected' event handler. %O`, err);
            });
        };
